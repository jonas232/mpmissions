...
RYD_CFF_TGT = 
	...
	foreach _enemies;
		...
		if not ((vehicle _CL) == _CL) then 
			{
			_veh = vehicle _CL;
			if (typeOf _veh in (RydFFE_SPMortar + RydFFE_Mortar + RydFFE_Rocket)) then {_artFactor = 10} else {_vehFactor = 500 + (rating _veh)};
			};
		...
		_nearImp = (getPosATL _CL) nearEntities [["CAManBase","AllVehicles","Strategic","WarfareBBaseStructure","Fortress"],100];
			...
			if (((side _x) getFriend (side _CL) >= 0.6) and not (_x isKindOf "civilian")) then 
				...
				if not (_x == _vh) then 
					{
					_crowdFactor = _crowdFactor + 0.2;
					if (typeOf _vh in (RydFFE_SPMortar + RydFFE_Mortar + RydFFE_Rocket)) then 
						{
						_crowdFactor = _crowdFactor + 0.2
						}
					}
...
RYD_CFF_FFE = 
	...
	"_maxRange","_minRange","_isTaken","_batlead","_alive","_waitFor","_UL","_ammoC","_add","_myFO","_assumedPos","_eta","_bNme","_caller","_sent","_name","_adj","_assPos","_dstIT","_dstFT","_dstCI",
	"_rem","_change","_FOpos"];	
	...
	_bNme = _battery1 getVariable ["RYD_JR_BatteryName","Hammer"];
	...
	_amount2 = _amount - _amount1;
		...
		_cached = false;
			...
			{
			if (_x in RYD_JR_Cached) exitWith
				{
				_cached = true
				}
			}
		...
		if (_cached) exitWith {_waitFor = false};
		...
		if ((count _phaseF) == 2) then
			...
			if (_x == 1) then
				...
				if (((side _caller) in RYD_JR_Freq) and {"ItemRadio" in (assignedItems player)}) then
					{
					if (_batLead1 isEqualTo _myFO) exitWith {};
					...
					if not (_finalImpact isEqualTo [0,0,0]) then
						{
						_assPos = (getPosATL _target);
						if not (isNull _myFO) then
							{
							_assPos = _myFO getHideFrom _target;
							_assPos set [2,0];
							
							if not (_assPos isEqualTo [0,0,0]) then
								{
								_FOpos = position _myFO;
								
								_dstIT = round (_assPos distance _finalImpact);
								_dstFT = round (_myFO distance _assPos);
								_dstCI = round (_myFO distance _finalImpact);
								_rem = _dstIT mod 50;
								_change = _dstIT - _rem;
								
								if (_rem > 25) then {_change = _change + 50};
								
								_angle = [_FOpos,_assPos,0] call RYD_JR_AngTowards;
								...
								if (_change > 0) then
									...
									switch (true) do
										{
										case ((_dstCI - _dstFT) > (_dstIT/2)) : {_adj = format [". Minus %1",_change]};
										case ((_dstCI - _dstFT) < -(_dstIT/2)) : {_adj = format [". Plus %1",_change]};
										default
											{
											_adj = format [". %2 %1",_change,_move]
											}
										};
									}
								}
					...
					_sent = _name + " to: " + _bNme +  _adj + ". Fire for effect! Over";
		...
		_eta = -1;
			...
			switch (true) do
				{
				case (isNil {_x}) : {_battery set [_foreachIndex,0]};
				case (isNull _x) : {_battery set [_foreachIndex,0]};
				case (({((alive _x) and not (_x == (vehicle _x)))} count (units _x)) < 1) : {_battery set [_foreachIndex,0]};
				}
			}
		foreach _battery;
		...
		_finalimpact = [(_impactpos select 0) + (random (2 * _acc)) - _acc,(_impactpos select 1) + (random (2 * _acc)) - _acc,0];
		...
		_distance = _impactPos vectorDistance _finalimpact;
		...
		if (_Debug) then 
			...
			_distance = _impactPos vectorDistance _finalimpact;
			_distance2 = _impactPos vectorDistance (getposATL (vehicle _batlead1));
		...
		[_battery,_distance,_eta,_ammoG,_batlead,_target,_markers,_bNme,_myFO] spawn
			{
			...
			_bNme = _this select 7;
			_myFO = _this select 8;
			...
			while {(not (_rEta < 5) and not (_TOF > 200) and (_alive))} do
				...
				if (_cached) exitWith {_alive = false};
			...
			if ((count _markers) > 0) then
				...
				_caller = leader _battery1;
				
				if (((side _caller) in RYD_JR_Freq) and {"ItemRadio" in (assignedItems player)}) then
					{
					if (_caller isEqualTo _myFO) exitWith {};
					
					if ((_caller distance player) < 10000) then
						{																
						_sent = _bNme + ": splash. Out";
		...
		if not (_alive) then 
			...
			{
			if (({({alive _x} count (units _x)) > 0} count _battery) > 0) then
				{
				_caller = leader (_battery select 0);	
				...
				if (((side _caller) in RYD_JR_Freq) and {"ItemRadio" in (assignedItems player)}) then
					...
					if ((_caller distance player) < 10000) then
						...
						if ((_where distance [0,0,0]) == 0) then
							{
							_where = mapGridPosition _target
							}
						else
							{
						...
						_sent = _bNme + " to: " + (name _myFO) + ". Rounds complete. Out";
	...
	if (_waitFor) then
		...
		{
		(group _myFO) setVariable ["RYD_JR_ArtyRefused",time];
		...
		if (({({alive _x} count (units _x)) > 0} count _battery) > 0) then
			...
			if (((side _caller) in RYD_JR_Freq) and {"ItemRadio" in (assignedItems player)}) then
				...
				if ((_caller distance player) < 10000) then
					...
					_sent = _bNme + " to: " + (name _myFO) + ". Negative. Fire mission impossible. Out";
...
RYD_CFF = 
	...
	private ["_artG","_knEnemies","_enArmor","_friends","_Debug","_CFFMissions","_tgt","_ammo","_bArr","_possible","_amount","_myFO","_assumedPos","_amnt","_name","_caller","_sent"];
	...
	for "_i" from 1 to _CFFMissions do
		...
		if not (isNull _tgt) then
			...
			_myFO = _tgt getVariable ["RydFFE_MyFO",objNull];
			
			if (isNull _myFO) exitWith {};
			if not (alive _myFO) exitWith {};
			
			_lastRej = (group _myFO) getVariable ["RYD_JR_ArtyRefused",0];
									
			if ((time - _lastRej) < 120) exitWith {};

			_assumedPos = (getPosATL _tgt);
			_assumedPos = _myFO getHideFrom _tgt;
			_assumedPos set [2,0];
				
			if ((_assumedPos distance [0,0,0]) == 0) then
				{
				_assumedPos = (getPosATL _tgt)
				};
				
			if ({((_assumedPos distance _x) < (120 * (1 + overcast)))} count (units (group _myFO)) > 0) exitWith {};
			...
			if (RYD_JR_ACIntense > 0) then
				...
				_artG = [];
					...
					{
					if ((side _x) isEqualTo _foSide) then
						{
						_artG pushBack _x
						}
					}
			...
			if ((count _artG) < 1) exitWith {};

			_bArr = [_assumedPos,_artG,_ammo,_amnt,objNull] call RYD_ArtyMission;
			...
			if (_possible) then
				...
				_bNme = ((_bArr select 1) select 0) getVariable ["RYD_JR_BatteryName","Hammer"];
				
				_caller = _myFO;			
				if (((side _caller) in RYD_JR_Freq) and {"ItemRadio" in (assignedItems player)}) then
					{	
					_battery1 = (_bArr select 1) select 0;
					_batLead1 = leader _battery1;					
					if (_batLead1 isEqualTo _myFO) exitWith {};
					...
					_where = _caller getHideFrom _tgt;
					_where set [2,0];
					
					if ((_where distance [0,0,0]) == 0) then
						{
						_where = mapGridPosition _tgt
						}
					else
						{
						_where = mapGridPosition _where
					...
					_sent = _name + " to: " + _bNme + ": requesting adjust fire mission at " + _where + ". Over";
...
RYD_CFF_Fire = 
	{
	private ["_battery","_pos","_ammo","_amount","_guns","_vh","_mags","_amount0","_eta","_alive","_available","_perGun","_rest","_aGuns","_perGun1","_shots","_toFire","_rest0","_bad","_ammoC","_ws"];
	...
	if (_bad) exitWith {-1};
		...
		switch (true) do
			{
			case (isNil {_x}) : {_guns set [_foreachIndex,0]};
			case (isNull _x) : {_guns set [_foreachIndex,0]};
			case not (alive _x) : {_guns set [_foreachIndex,0]};
			}
		}
	foreach _guns;
