...
case "eng":
{
	...
	if (_loadout == 1) then {
		...
		(unitBackpack _unit) addItemCargoGlobal [_slam,2];
...
case "div":
{
	...
	if (_loadout == 1) then {
		...
		(unitBackpack _unit) addMagazineCargoGlobal [_mgrenade, 1];
		(unitBackpack _unit) addMagazineCargoGlobal [_smokegrenade, 1];
		(unitBackpack _unit) addMagazineCargoGlobal [_cgrenade,4];
		(unitBackpack _unit) addMagazineCargoGlobal [_flashbang,4];
	};
	// LOADOUT: HEAVY
	if (_loadout == 2) then {
		_unit addBackpack _bagmediumdiver;
...
case "cc":
{
	...
	clearMagazineCargoGlobal (unitBackpack _unit);
	(unitBackpack _unit) addItemCargoGlobal ["ToolKit",1];
	(unitBackpack _unit) addItemCargoGlobal ["rhsusf_opscore_paint_pelt_nsw_cam", 1];
};