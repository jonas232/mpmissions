...
case "eng":
{
	...
	if (_loadout == 1) then {
		...
		(unitBackpack _unit) addItemCargoGlobal [_defusal,1];
		(unitBackpack _unit) addItemCargoGlobal [_strobe,4];
		(unitBackpack _unit) addItemCargoGlobal [_redchem,8];
		(unitBackpack _unit) addItemCargoGlobal [_detector,1];
		(unitBackpack _unit) addItemCargoGlobal [_helddetector,1];
	};
	// LOADOUT: HEAVY
	if (_loadout == 2) then {
		_unit addBackpack _baglarge;
...
case "div":
{
	...
	if (_loadout == 1) then {
		...
		(unitBackpack _unit) addItemCargoGlobal [_snscope,1];
		(unitBackpack _unit) addItemCargoGlobal [_acog,2];
	};
	// LOADOUT: HEAVY
	if (_loadout == 2) then {
		_unit addBackpack _bagmediumdiver;
